...
System został zaprojektowany jako aplikacja sieciowa składająca się z trzech głównych części: aplikacji klienckiej, aplikacji serwerowej oraz modułu analitycznego napisanego w języku Python. W dalszej części pracy aplikacja kliencka określana jest również jako frontend, natomiast aplikacja serwerowa jako backend. Pojęcia te są stosowane ze względu na ich powszechne użycie w projektach aplikacji webowych oraz bezpośrednie odniesienie do struktury zaimplementowanego projektu.

Frontend odpowiada za kontakt z użytkownikiem, czyli wybór pliku wideo, przesłanie go do analizy oraz prezentację wyniku. Backend przyjmuje nagranie, zapisuje informacje o analizie i uruchamia moduł analityczny. Moduł analityczny przetwarza nagranie oraz zwraca wynik w postaci danych, które mogą zostać zapisane po stronie serwera i pokazane w interfejsie użytkownika.

    
\subsection{Ogólna struktura systemu}

Podstawowym założeniem projektowym było rozdzielenie części odpowiedzialnej za obsługę użytkownika od części odpowiedzialnej za analizę nagrania. Użytkownik nie uruchamia bezpośrednio programu analizującego wideo. Korzysta jedynie z aplikacji webowej, przez którą wybiera plik i otrzymuje wynik. Backend pełni w tym układzie rolę pośrednika pomiędzy interfejsem użytkownika a modułem analitycznym.
...
\item \textbf{moduł analityczny} -- oddzielny program, odpowiedzialny za właściwą analizę nagrania.
\end{itemize}

Taki podział odpowiada rzeczywistej strukturze projektu. Frontend i backend są oddzielnymi częściami aplikacji webowej, natomiast moduł analityczny znajduje się poza kodem odpowiedzialnym za obsługę żądań HTTP. Dzięki temu kod związany z analizą ruchu nie jest mieszany z kodem odpowiedzialnym za interfejs użytkownika oraz komunikację sieciową.
\newpage
...
Interfejs użytkownika powinien w pierwszej kolejności prezentować wynik ogólny, a dopiero później szczegóły dotyczące poszczególnych powtórzeń. Jest to ważne, ponieważ użytkownik nie powinien być od razu zasypany wszystkimi danymi liczbowymi. Szczegółowe informacje mają pełnić funkcję pomocniczą i pozwalać na dokładniejsze przejrzenie wyniku wtedy, gdy użytkownik chce sprawdzić, skąd wynika dana ocena.

\subsection{Aplikacja serwerowa}

Backend odpowiada za przyjęcie nagrania przesłanego z frontendu oraz za uruchomienie analizy. Po otrzymaniu żądania backend tworzy rekord analizy na podstawie przesłanych danych i zapisuje w nim plik wideo. Początkowo analiza otrzymuje status oczekujący. Następnie status zostaje zmieniony na informujący o rozpoczęciu przetwarzania, a backend przekazuje ścieżkę zapisanego pliku do modułu analitycznego. Następnie status analizy zmieniany jest na informujący o rozpoczęciu przetwarzania, a backend przekazuje ścieżkę zapisanego pliku do modułu analitycznego.

Dla każdej przesłanej analizy przechowywany jest status, który informuje, czy analiza oczekuje na wykonanie, jest przetwarzana, zakończyła się poprawnie, albo zakończyła się błędem. Pozwala to odróżnić analizę oczekującą, analizę w trakcie przetwarzania, analizę zakończoną poprawnie oraz analizę zakończoną błędem. Oprócz statusu przechowywany jest także plik wideo, wynik analizy, ewentualny komunikat błędu oraz czas rozpoczęcia i zakończenia przetwarzania.
...
Backend nie wykonuje samodzielnie obliczeń biomechanicznych. Nie wykrywa punktów kluczowych, nie dzieli nagrania na powtórzenia i nie ocenia techniki. Te zadania zostały pozostawione modułowi analitycznemu. Dzięki temu część serwerowa pozostaje odpowiedzialna głównie za obsługę żądań, zapis danych i połączenie z częścią analizującą nagranie.

\subsection{Moduł analityczny DO OCENY}

Moduł analityczny jest częścią systemu odpowiedzialną za przetworzenie nagrania wideo. W sensie implementacyjnym jest to wydzielona część projektu napisana w języku Python, uruchamiana z poziomu backendu po przesłaniu nagrania przez użytkownika. Wejściem modułu jest ścieżka do pliku wideo, a wyjściem struktura danych zawierająca wynik analizy.

W module analitycznym wykorzystano gotowy model estymacji pozy człowieka \cite{mediapipe_pose_landmarker}. Model ten odpowiada za wykrywanie punktów kluczowych sylwetki w kolejnych klatkach nagrania. Dalsze przetwarzanie jest już realizowane przez kod przygotowany w ramach projektu. Obejmuje ono wybór potrzebnych punktów, obliczenie parametrów opisujących ruch, wykrycie powtórzeń oraz ocenę techniki.

W module analizy wideo zastosowano wariant Heavy modelu MediaPipe Pose Landmarker. Spośród dostępnych wariantów Lite, Full i Heavy charakteryzuje się on największym kosztem obliczeniowym, ale jednocześnie osiąga najlepsze wyniki estymacji punktów kluczowych w testach przedstawionych przez twórców rozwiązania \cite{mediapipe_pose_docs}. Wybór tego wariantu wynika z charakteru projektowanego systemu. Analiza nie jest wykonywana w czasie rzeczywistym. Użytkownik najpierw przesyła nagranie do aplikacji, a następnie materiał jest przetwarzany po stronie serwera. Minimalizacja opóźnienia pojedynczego wywołania modelu nie stanowi więc podstawowego wymagania systemu. Większe znaczenie ma uzyskanie możliwie dobrej jakości estymacji punktów, ponieważ ich współrzędne stanowią dane wejściowe dla kolejnych etapów analizy biomechanicznej.
...
Wewnątrz modułu analitycznego można wyróżnić kilka etapów, ale nie są one osobnymi modułami systemu widocznymi z zewnątrz. Są to raczej kolejne kroki przetwarzania wykonywane w ramach jednej części projektu. Szczegółowy opis ich implementacji zostanie przedstawiony w kolejnym rozdziale.
